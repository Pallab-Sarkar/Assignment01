import { create, getAll, getOne, updateByCondn, deleteItem as _deleteItem } from "../dao/item-dao.js";
import { constants } from "../config/constants.js";

//Returns Item detail & Creating item into DB
export async function createItem(req,data){
    return new Promise(async (resolve,reject) => {
        ...
        try {
            ...
            if(result){
                ...
                reject({message:"Item could not be created"})
            }
        } catch (error) {
            reject(error)
        }
...
//Update item
export async function updateItem(req,updateFields){
    return new Promise(async (resolve,reject) => {
        if (!(req.params && req.params.id)) {
            return reject({status: constants.HTML_STATUS_CODE.INVALID_DATA, message:'Invalid item id !!'});
        }
        if (!updateFields) {
            return reject({status: constants.HTML_STATUS_CODE.INVALID_DATA, message:'Invalid Data !!'});
        }
        ...
        if(!itemDetail){
            return reject({status: constants.HTML_STATUS_CODE.INVALID_DATA, message:'Item does not exist !!'});
        }
        ...
        try {
            ...
            if (!update || update.modifiedCount !== 1) {
                return reject({status: constants.HTML_STATUS_CODE.INVALID_DATA, message:'Unable to update Item!!'});
            }
...
export async function deleteItem(req ,params){
    return new Promise(async (resolve,reject) => {
        if (!params) {
            return reject({status: constants.HTML_STATUS_CODE.INVALID_DATA, message:'Invalid Item ID !!'});
        }
        ...
        const itemDetail = await getOne({_id: params, 'auditFields.isDeleted': false })
        if (!itemDetail) {
            return reject({status: constants.HTML_STATUS_CODE.NOT_FOUND, message:'Item does not exist !!'});
        }
    
        try {
            const result = await _deleteItem({_id: req.params.id});
            if (!result || result.modifiedCount !== 1) {
                return reject({status: constants.HTML_STATUS_CODE.INVALID_DATA, message:'Unable to delete Item!!'});
            }
