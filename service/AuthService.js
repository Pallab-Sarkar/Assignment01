    ...
    return new Promise( async(resolve,reject) => {
        try {
            ...
            const userData = {
                ...
                password: bodyData.password || generate(6),
                role: role,
                otp: generate({
                    length: 4,
                    charset: 'numeric'
            ...
            if(existingUser){
                reject({ message: "User already exists with same email !" });
            }
            ...
            if (userData.password.length <= 5) {
                ...
                if (result) {
                    ...
                    reject({ message: "Error while creating a new User !" })
                }
            }
    ...
    return new Promise( async (resolve, reject) => {
        try {
            ...
            if (!user) {
                ...
                reject({ message: "User has been deleted!" });

            } else if (user.isBlocked) {
                reject({ message: "User has been blocked!", status:403 });

            } else if (!(user.auditFields.isActive && user.isVerified)) {
                reject({ message: "User not active!", data: { isVerified: user.isVerified, auditFields: user.auditFields }});
            } else {
                let updateFields = {};
        ...
        try {
            ...
            if (!existingUser) {
                ...
                if (user && user.modifiedCount !== 0) {
                    ...
                    reject({ message: "Unable to activate user" });
                }
